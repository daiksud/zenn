...
emoji: 🔓
published: true
title: サービスにAIエージェント機能はいらない。必要なのは「個人のAIエージェントへの解放」だ
...
## 💰 おまけ：AIのコストは「ユーザー持ち」になる

さらに、サービスにAIエージェント機能を搭載しないことには、**サービス提供側にとっての金銭的なメリット** もあります。

自前でAI機能を抱えない以上、**AIにかかる費用（モデルの利用料など）を負担せずに済む** のです。AIエージェントを動かす費用は、ユーザーが「自分のエージェントの費用」として支払ってくれています。

:::message
だからこそサービス側がやるべきは、AIを抱え込んでコストを負担することではなく、**ユーザーのAIエージェントに対して、必要な情報をどれだけ十分かつスムーズに提供できるか** に尽きます。
:::
